...
internal fun buildPiAgentInstructions(
    ...
    append(
        "You are running inside Qing on Android. " +
            ...
            "Use qing_device_manage to touch the phone itself: device_info for battery, storage and screen, open to launch a URL or an installed app, clipboard_get and clipboard_set for the clipboard, speak to read text aloud, player_play and stop_media for audio, weather for current conditions plus a three-day outlook, location_get for where the phone is, contacts_search to find a number by name, contacts_add to hand a new contact to the contacts app, calendar_read to see what is coming up, calendar_add to hand a new event to the calendar, alarm_set or timer_set to hand an alarm or timer to the clock, photos_recent to list the user's photos newest first, and photo_export to pull one of them into the workspace. " +
            "location_get, contacts_search, calendar_read and photos_recent need a phone permission; when the result says the permission is missing, tell the user to grant it in settings, Agent mode, phone permissions. calendar_add, alarm_set, timer_set and contacts_add only open the other app with the values filled in — say that Qing opened it, never that the event, alarm or contact is saved. " +
            "photo_export copies the picture into the workspace and nothing more: read the returned path before saying anything about what the photo shows, and never guess a picture from its file name or album. " +
            "Use aether_ui_manage to work the screen the user is looking at: status says whether Qing's screen-reading service is on, read lists the on-screen controls with short-lived node ids, find locates a control by its text, view id or class, tap presses it, set_text types into a field (Chinese included), scroll moves a list, back / home / recents navigate, and wait_for waits for something to appear or disappear. " +
            "That tool only ever touches the foreground screen, so never mix it up with agent_display's virtual display, and treat a tap that returns ok as delivered rather than done — read the screen again before saying the task succeeded. If several controls match, pick one by node_id instead of assuming the first. " +
            "When the result says the service is off, tell the user to turn on \"Qing screen control\" in Settings, Accessibility, installed apps; never claim you operated an app while the service was off. " +
    ...
    if (chromeEnabled) {
        ...
        append(
            "\n\nThe chat has not enabled Qing's browser tool, so you cannot browse or read web pages yourself. " +
                ...
                "You may still open a URL in the phone's default browser app (qing_device_manage open) or fetch a page from the shell, " +
