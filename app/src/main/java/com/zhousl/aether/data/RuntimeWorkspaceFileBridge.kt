...
class RuntimeWorkspaceFileBridge(
    ...
    private val embeddedTermuxRuntime: TermuxEmbeddedRuntime,
) {
    ...
    suspend fun importAttachmentToWorkspace(
        ...
        runtimeOrder.forEach { runtimeId ->
            val result = when (runtimeId) {
                ...
                LocalRuntimeId.EmbeddedTermux -> importAttachmentToEmbeddedTermuxWorkspace(
                    sourceUri = sourceUri,
                    attachmentId = attachmentId,
                    displayName = displayName,
                    onProgress = onProgress,
    ...
    suspend fun readWorkspaceFile(
        ...
        val resolvedWorkingDirectory = workingDirectory.ifBlank {
            when (runtimeId) {
                ...
                LocalRuntimeId.EmbeddedTermux -> embeddedTermuxRuntime.workspaceRoot
            }
        }
    ...
    private suspend fun readWorkspaceFile(
        ...
        return when (runtimeId) {
            ...
            LocalRuntimeId.EmbeddedTermux -> readEmbeddedTermuxWorkspaceFile(
                path = path,
                workingDirectory = workingDirectory,
                byteLimit = byteLimit,
            )
    ...
    suspend fun writeWorkspaceBytes(
        ...
        return when (runtimeId) {
            ...
            LocalRuntimeId.EmbeddedTermux -> writeEmbeddedTermuxWorkspaceBytes(
                path = normalizedPath,
                workingDirectory = workspaceDirectory,
                bytes = bytes,
            )
    ...
    suspend fun saveWorkspaceFileToDocument(
        ...
        return when (runtimeId) {
            ...
            LocalRuntimeId.EmbeddedTermux -> saveEmbeddedTermuxWorkspaceFileToDocument(
                path = normalizedPath,
                workingDirectory = workspaceDirectory,
                destinationUri = destinationUri,
                byteLimit = byteLimit,
    ...
    private suspend fun importAttachmentToEmbeddedTermuxWorkspace(
        sourceUri: Uri,
        attachmentId: String,
        displayName: String,
        onProgress: (WorkspaceImportProgress) -> Unit,
        ...
        runCatching {
            require(embeddedTermuxRuntime.inspectSetup().isReady) { "Embedded Termux runtime is not ready." }
            val safeFileName = buildRuntimeWorkspaceFileName(attachmentId, displayName)
            val destinationPath = "${embeddedTermuxRuntime.workspaceRoot}/uploads/$safeFileName"
            val resolved = embeddedTermuxRuntime.resolveWorkspaceHostPath(destinationPath)
                ?: error("Unable to resolve the embedded Termux workspace path.")
            resolved.hostFile.parentFile?.mkdirs()
    ...
    private suspend fun readEmbeddedTermuxWorkspaceFile(
        path: String,
        workingDirectory: String,
        byteLimit: Int,
    ): Result<WorkspaceFilePayload> = withContext(Dispatchers.IO) {
        runCatching {
            require(byteLimit > 0) { "byteLimit must be greater than 0." }
            val resolved = embeddedTermuxRuntime.resolveWorkspaceHostPath(path, workingDirectory)
                ?: error("Path is outside the embedded Termux workspace: $path")
            val file = resolved.hostFile
            require(file.isFile) { "File was not found in the embedded Termux workspace: ${resolved.guestPath}" }
            val sizeBytes = file.length()
            require(sizeBytes <= byteLimit) {
                "File is larger than $byteLimit bytes: ${resolved.guestPath}"
            }
    ...
    private suspend fun writeEmbeddedTermuxWorkspaceBytes(
        path: String,
        workingDirectory: String,
        bytes: ByteArray,
    ): Result<Long> = withContext(Dispatchers.IO) {
    ...
    private suspend fun saveEmbeddedTermuxWorkspaceFileToDocument(
        path: String,
        workingDirectory: String,
        destinationUri: Uri,
        byteLimit: Int,
        ...
        runCatching {
            val resolved = embeddedTermuxRuntime.resolveWorkspaceHostPath(path, workingDirectory)
                ?: error("Path is outside the embedded Termux workspace: $path")
            val source = resolved.hostFile
            require(source.isFile) { "File was not found in the embedded Termux workspace: ${resolved.guestPath}" }
            require(source.length() <= byteLimit) {
                "File is larger than $byteLimit bytes: ${resolved.guestPath}"
            }
            val resolver = source.takeIf { it.canRead() }
